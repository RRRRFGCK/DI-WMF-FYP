...
from exact_occlusion import vectorized_occlusion, self_test as test_occlusion
...
SEVERITIES = [.05, .1, .2, .3]
NOISE_SEEDS = [0, 1, 2]
ORIGINAL_CORRUPTION = corrupt.apply_corruption_batch


def identical_fast_corruption(pixels, name, severity, generator):
    if name == "occlusion":
        return vectorized_occlusion(pixels, severity, generator)
    return ORIGINAL_CORRUPTION(pixels, name, severity, generator)
...
def evaluate_one(folder, output, cache, device):
    ...
    source_files = [Path(__file__), HERE / "exact_occlusion.py", *sorted((HERE / "evaluation_source").glob("*.py")),
    ...
    metadata = {"finished_utc": datetime.now(timezone.utc).isoformat(), "wall_seconds": time.perf_counter() - started,
                ...
                "occlusion_implementation": "vectorized mask; pixel and generator state equality verified against original loop",
...
def main():
    ...
    if args.self_test:
        tests = test_occlusion(ORIGINAL_CORRUPTION, device)
        dump(HERE / "exact_occlusion_test.json", {"passed": True, "device": str(device), "torch": torch.__version__, "tests": tests})
        print(json.dumps({"occlusion_equivalence_tests_passed": len(tests)}), flush=True)
        return
    assert (HERE / "exact_occlusion_test.json").exists(), "Run --self-test first"
    corrupt.apply_corruption_batch = identical_fast_corruption
    completed_now = 0
    while True:
